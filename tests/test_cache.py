...
class TestCache(unittest.TestCase):
    ...
    def test_methods_cached_across_calls(self):
        methods_list_call_1 = get_methods(device=None, methods=["echo"])
        methods_list_call_2 = get_methods(device=None, methods=["echo"])
    ...
    def test_mis_filter_created_with_default_device(self):
        ...
        mis_filter = get_mis_filter(device=None)
        self.assertIsNotNone(mis_filter)

    def test_mis_settings_update_without_reload(self):
        mis_filter = get_mis_filter(device=None, min_score=0.5)
        mis_filter_updated = get_mis_filter(device=None, min_score=0.9)
    ...
    def test_clear_cache_forces_new_instances(self):
        methods_list_before = get_methods(device=None, methods=["echo"])
        mis_filter_before = get_mis_filter(device=None)
        clear_cache()
        self.assertIsNot(get_methods(device=None, methods=["echo"])[0], methods_list_before[0])
        self.assertIsNot(get_mis_filter(device=None), mis_filter_before)
